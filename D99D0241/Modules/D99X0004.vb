...
''' </summary>
''' <remarks></remarks>
Public Module D99X0004
    Public gbUnicode As Boolean = True ' False
...
#Region "Các hàm liên quan đến ConvertFont"
    ...
    '        C1Grid.Splits(i).DisplayColumns(dc.DataField).HeadingStyle.Font = FontUnicode(bUseUnicode, C1Grid.Splits(i).DisplayColumns(dc.DataField).HeadingStyle.Font.Style, C1Grid.Splits(i).DisplayColumns(dc.DataField).HeadingStyle.Font.Size)
    ...
    ''' <summary>
    ''' Chuyển đổi chuỗi ký tự có dấu sang không dấu
    ''' </summary>
    ''' <param name="sText"></param>
    ''' <remarks></remarks>
    Public Function ConvertUnicodeToCharactor(ByVal sText As String) As String
        Dim sRet As String = sText

        If sText.Trim = "" Then Return ""

        sRet = sRet.Replace("á", "a")
        sRet = sRet.Replace("à", "a")
        sRet = sRet.Replace("ả", "a")
        sRet = sRet.Replace("ã", "a")
        sRet = sRet.Replace("ạ", "a")

        sRet = sRet.Replace("ă", "a")
        sRet = sRet.Replace("ắ", "a")
        sRet = sRet.Replace("ằ", "a")
        sRet = sRet.Replace("ẳ", "a")
        sRet = sRet.Replace("ẵ", "a")
        sRet = sRet.Replace("ặ", "a")

        sRet = sRet.Replace("â", "a")
        sRet = sRet.Replace("ấ", "a")
        sRet = sRet.Replace("ầ", "a")
        sRet = sRet.Replace("ẩ", "a")
        sRet = sRet.Replace("ẫ", "a")
        sRet = sRet.Replace("ậ", "a")
        '------------------------------------------------------------------
        sRet = sRet.Replace("é", "e")
        sRet = sRet.Replace("è", "e")
        sRet = sRet.Replace("ẻ", "e")
        sRet = sRet.Replace("ẽ", "e")
        sRet = sRet.Replace("ẹ", "e")
        sRet = sRet.Replace("ê", "e")
        sRet = sRet.Replace("ế", "e")
        sRet = sRet.Replace("ề", "e")
        sRet = sRet.Replace("ể", "e")
        sRet = sRet.Replace("ễ", "e")
        sRet = sRet.Replace("ệ", "e")
        '------------------------------------------------------------------
        sRet = sRet.Replace("í", "i")
        sRet = sRet.Replace("ì", "i")
        sRet = sRet.Replace("ỉ", "i")
        sRet = sRet.Replace("ĩ", "i")
        sRet = sRet.Replace("ị", "i")
        '------------------------------------------------------------------
        sRet = sRet.Replace("ó", "o")
        sRet = sRet.Replace("ò", "o")
        sRet = sRet.Replace("ỏ", "o")
        sRet = sRet.Replace("õ", "o")
        sRet = sRet.Replace("ọ", "o")
        sRet = sRet.Replace("ô", "o")
        sRet = sRet.Replace("ố", "o")
        sRet = sRet.Replace("ồ", "o")
        sRet = sRet.Replace("ổ", "o")
        sRet = sRet.Replace("ỗ", "o")
        sRet = sRet.Replace("ộ", "o")
        sRet = sRet.Replace("ơ", "o")
        sRet = sRet.Replace("ớ", "o")
        sRet = sRet.Replace("ờ", "o")
        sRet = sRet.Replace("ở", "o")
        sRet = sRet.Replace("ỡ", "o")
        sRet = sRet.Replace("ợ", "o")

        '------------------------------------------------------------------
        sRet = sRet.Replace("ú", "u")
        sRet = sRet.Replace("ù", "u")
        sRet = sRet.Replace("ủ", "u")
        sRet = sRet.Replace("ũ", "u")
        sRet = sRet.Replace("ụ", "u")
        sRet = sRet.Replace("ư", "u")
        sRet = sRet.Replace("ứ", "u")
        sRet = sRet.Replace("ừ", "u")
        sRet = sRet.Replace("ử", "u")
        sRet = sRet.Replace("ữ", "u")
        sRet = sRet.Replace("ự", "u")
        '------------------------------------------------------------------
        sRet = sRet.Replace("ý", "y")
        sRet = sRet.Replace("ỳ", "y")
        sRet = sRet.Replace("ỷ", "y")
        sRet = sRet.Replace("ỹ", "y")
        sRet = sRet.Replace("ỵ", "y")
        '------------------------------------------------------------------
        sRet = sRet.Replace("đ", "d")
        sRet = sRet.Replace("Đ", "D")
        '------------------------------------------------------------------
        sRet = sRet.Replace("Á", "A")
        sRet = sRet.Replace("À", "A")
    ...
    ''' <summary>
    ''' Chuyển dữ liệu từ có dấu sang dữ liệu không dấu (Mode Unicode)
    ''' </summary>
    ''' <param name="dtData"></param>
    ''' <param name="sColName">Tên cột cần chuyển</param>
    ''' <param name="sColNameUnsign">Tên cột chứa dữ liệu không dấu. Nếu không truyền thì mặc định sColName__Unsign</param>
    ''' <remarks></remarks>
    Public Sub ConvertDataToUnsign(ByRef dtData As DataTable, ByVal sColName As String, Optional ByVal sColNameUnsign As String = "")
        If dtData Is Nothing Then Exit Sub

        If sColNameUnsign = "" Then sColNameUnsign = sColName & "_Unsign"

        If Not dtData.Columns.Contains(sColNameUnsign) Then
            dtData.Columns.Add(sColNameUnsign)
        End If
        If geLanguage <> EnumLanguage.Vietnamese Then Exit Sub
        If Not gbUnicode Then Exit Sub
        For Each row As DataRow In dtData.Rows
            row(sColNameUnsign) = ConvertUnicodeToCharactor(row(sColName).ToString)
        Next
    End Sub
...
#Region "Lấy biến CodeTable theo sản phẩm"
    ...
    ''' <remarks></remarks>
    <DebuggerStepThrough()> _
    Public Sub GetCodeTable() 'Bỏ nội dung này trên 4.1
        ''Update 04/02/2015: bỏ đoạn code này.
        'Kiểm tra tồn tại bảng D91T8888
        'Dim iDisplayProduct As Integer = 0
        'Dim sTable As String = "D91T8888"

        ''Kiểm tra tồn tại bảng D91T8888
        'Dim sSQL As String = ""
        'sSQL &= " SELECT TOP 1 1  FROM SYSOBJECTS WHERE ID = OBJECT_ID(N'[D91T8888]') AND OBJECTPROPERTY(ID, N'IsTable') = 1"
        'If Not ExistRecord(sSQL) Then ' Chưa tồn tại bảng D91T8888
        '    'Lấy dữ liệu từ bảng D00T0010
        '    sTable = "lemonsys.dbo.D00T0010"
        'End If

        'Dim sDisplayProduct As String = ReturnScalar("SELECT TOP 1 DisplayProduct FROM " & sTable & " Where ModuleID ='D00' And UserID = " & SQLString(gsUserID))
        'If sDisplayProduct = "" Then
        '    iDisplayProduct = CInt(GetSetting("Lemon3", "Settings", "DisplayProduct", "0"))
        'Else
        '    iDisplayProduct = L3Int(sDisplayProduct)
        'End If
        'geDisplayProduct = CType(iDisplayProduct, EnumDisplayProduct)
        'Bổ sung in báo cáo VNI - Tạm thời dùng câu Select, sẽ bổ sung store sau.
        'gbPrintVNI = L3Bool(ReturnScalar("Select IsPrintVNI from D91T0025  WITH(NOLOCK) ")) 'Bỏ trên 4.1
        'If gbIsDesktop Then gbUnicode = True : Exit Sub 'bổ sung 08/02/2017 lỗi khi gọi desktop nhưng txt bị lỗi font vì user này sử dụng Lemon3 là VNI
        ''Update 12/05/2015: kiểm tra nếu SP là L3HR thì luôn luôn là Unicode
        'If geDisplayProduct = EnumDisplayProduct.L3HR Then
        '    gbUnicode = True
        'Else
        '    Dim iDisplayProduct As Integer = 0
        '    Dim sSQL As String = ""
        '    iDisplayProduct = geDisplayProduct
        '    sSQL = SQLStoreD91P3001(iDisplayProduct)
...
#Region "Các hàm liên quan nhập bằng Unicode"
    ...
    Public Function SQLStringUnicode(ByVal txtName As System.Windows.Forms.TextBox, ByVal bSaveUnicode As Boolean) As String
        ...
        'Update 1/8/2014: 67734 do 1 máy nào đó dùng font Arial, chưa tìm ra nguyên nhân, tạm thời fix như vậy. 
        'Return SQLStringUnicode(txtName.Text, txtName.Font.Name.ToLower <> "lemon3", bSaveUnicode)
        Return SQLStringUnicode(txtName.Text, gbUnicode, bSaveUnicode)
    End Function
    ...
    Public Function SQLStringUnicode(ByVal Text As String) As String
        ...
        Return "N" & SQLString(Text.Trim) 'Sửa V4.1
    End Function
    ...
    Private Function SQLStringVni(ByVal Text As String) As String
        ' Return SQLString(ConvertUnicodeToVni(Text.Trim)) 'Bỏ trên V4.1
        'Update 20/08/2010: Khi đang nhập liệu ở Unicode thì không lưu cho trường VNI
        'Return SQLStringUnicode(Text)
        Return SQLString(Text.Trim)
    End Function
    ...
    Public Function UnicodeCaption(ByVal bUnicode As Boolean) As String
        Return ""
        ''Update 08/09/2014: nếu gọi từ Desktop thì caption form không có chữ Unicode.
        'If gbIsDesktop Then Return ""
    ...
    Public Function FontUnicode(Optional ByVal bUseUnicode As Boolean = True, Optional ByVal fontStyle As System.Drawing.FontStyle = FontStyle.Regular, Optional ByVal size As Single = 8.25) As System.Drawing.Font
        ...
        'Update 10/4/2014
        If size <= 8.25 Then 'Có trường hợp fontSize = 8.249999
            size = iSizeFont
        End If
        '  Return (New System.Drawing.Font(IIf(bUseUnicode, "Microsoft Sans Serif", "Lemon3").ToString, size, fontStyle, GraphicsUnit.Point))
        Return (New System.Drawing.Font("Microsoft Sans Serif", size, fontStyle, GraphicsUnit.Point)) 'sửa V41
    End Function
    ...
    Public Function UnicodeGetReportPath(ByVal bUnicode As Boolean, ByVal iReportLanguage As Byte, ByVal sValueCustom As String) As String
        If sValueCustom <> "" Then Return gsApplicationSetup & "\XCustom\"
        '  If bUnicode Then
        Return gsApplicationSetup & IIf(iReportLanguage = 0, "\Reports\", IIf(iReportLanguage = 1, "\Reports\VE-Reports\", "\Reports\E-Reports\")).ToString
        'Else
        'Return gsApplicationSetup & IIf(iReportLanguage = 0, "\XReports\", IIf(iReportLanguage = 1, "\XReports\VE-XReports\", "\XReports\E-XReports\")).ToString
        'End If
    End Function
    ...
    Public Function UnicodeConvertFont(ByVal ctrl As Control, ByVal bUseUnicode As Boolean) As Boolean
        ...
        ''Update 20/01/2015: lỗi cho TH C1Combo thiết kế có Font <>  EditorFont
        ''If ctrl.Font.Name = FontUnicode(bUseUnicode, ctrl.Font.Style, ctrl.Font.Size).Name Then
        ...
        '        ConvertFontTDBGrid(CType(ctrl, C1.Win.C1TrueDBGrid.C1TrueDBGrid), bUseUnicode, False) 'TH đúng font lưới nhưng phải convert font của Group
        '    ElseIf TypeOf (ctrl) Is C1.Win.C1List.C1Combo Then
        '        'Update 20/01/2015: sửa lỗi cho TH C1Combo thiết kế có Font <>  EditorFont
        '        Dim tdbc As C1.Win.C1List.C1Combo = CType(ctrl, C1.Win.C1List.C1Combo)
        '        If tdbc.EditorFont.Name <> tdbc.Font.Name Then tdbc.EditorFont = FontUnicode(bUseUnicode, tdbc.EditorFont.Style, tdbc.EditorFont.Size)
        '    End If
        '    Return False 'Chỉ convert 1 lần
        'End If
        ...
        '    ctrl.Font = FontUnicode(bUseUnicode, ctrl.Font.Style, ctrl.Font.Size)
        '    Return True
        'ElseIf TypeOf (ctrl) Is C1.Win.C1List.C1Combo Then
        '    Dim tdbc As C1.Win.C1List.C1Combo = CType(ctrl, C1.Win.C1List.C1Combo)
        '    tdbc.Font = FontUnicode(bUseUnicode, tdbc.Font.Style, tdbc.Font.Size)
        '    tdbc.EditorFont = FontUnicode(bUseUnicode, tdbc.EditorFont.Style, tdbc.EditorFont.Size)
        '    Return True
        'ElseIf TypeOf (ctrl) Is C1.Win.C1TrueDBGrid.C1TrueDBDropdown Then 'Bổ sung 25/07/2012
        '    'ctrl.Font = FontUnicode(bUseUnicode, ctrl.Font.Style, ctrl.Font.Size)
        '    Dim tdbd As C1.Win.C1TrueDBGrid.C1TrueDBDropdown = CType(ctrl, C1.Win.C1TrueDBGrid.C1TrueDBDropdown)
        '    tdbd.Font = FontUnicode(bUseUnicode, tdbd.Font.Style, tdbd.Font.Size)
        '    For i As Integer = 0 To tdbd.DisplayColumns.Count - 1
        '        tdbd.DisplayColumns(i).Style.Font = FontUnicode(bUseUnicode, tdbd.Font.Style, tdbd.Font.Size)
        '    Next
        '    Return True
        ...
        '    C1Grid.Font = FontUnicode(bUseUnicode, C1Grid.Font.Style, C1Grid.Font.Size)
    ...
    Private Sub ConvertFontTDBGrid(ByVal C1Grid As C1.Win.C1TrueDBGrid.C1TrueDBGrid, ByVal bUseUnicode As Boolean, Optional ByVal fontGrid As Boolean = True)
        ...
        '        If fontGrid Then
        '            'Modify 02/08/2012
        '            C1Grid.Splits(iSplit).DisplayColumns(index).Style.Font = FontUnicode(bUseUnicode, C1Grid.Splits(iSplit).DisplayColumns(index).Style.Font.Style, C1Grid.Splits(iSplit).DisplayColumns(index).Style.Font.Size)
        '            C1Grid.Splits(iSplit).DisplayColumns(index).EditorStyle.Font = FontUnicode(bUseUnicode, C1Grid.Splits(iSplit).DisplayColumns(index).EditorStyle.Font.Style, C1Grid.Splits(iSplit).DisplayColumns(index).EditorStyle.Font.Size)
        '        End If
        '        '**************
        '        If bGroupBy Then
        ...
        '            End If
        '            C1Grid.GroupStyle.Font = FontUnicode(bUseUnicode, C1Grid.GroupStyle.Font.Style, C1Grid.GroupStyle.Font.Size)
        '            C1Grid.Splits(iSplit).DisplayColumns(index).HeadingStyle.Font = FontUnicode(bUseUnicode, C1Grid.Splits(iSplit).DisplayColumns(index).HeadingStyle.Font.Style, C1Grid.Splits(iSplit).DisplayColumns(index).HeadingStyle.Font.Size)
        '        End If
        '        '  Exit For
        '        ' End If
        '    Next
