...
#Region "Tạo IGE cho khóa chính của bảng khi lưu dữ liệu, không lấy trong DLL"
    ...
    ''' <param name="Key1">module =xx</param>
    ''' <param name="Key2">2 ký tự tự do theo tài liệu</param>
    ''' <param name="Key3">gsStringKey</param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    ...
    Dim sSQL As String = "-- Sinh IGE cho khoa chinh " & vbCrLf
        ...
        Try
            ...
            Else 'Sinh IGE kiểu cũ
                ...
                Do
                    ...
                    'Luu Last key
                    'Update 20/03/2015: khong dung ham SaveLastKey
                    'SaveLastKey(sTableName, KeyString, LastKey - 1 + nRowIGE)
                    Dim sSQL As String = "----Luu New Last Key khi sinh IGE dang cu" & vbCrLf
                    sSQL &= SQLStoreD91P9111("", "", "", "", "", 1, 1, "", KeyString, 0, 1, LastKey - 1 + nRowIGE, 1, sTableName)
...
#Region "Sinh IGE có truyền chuỗi kết nối khác"


    Public Function CreateIGE_ConnectOther(ByVal sConnectionStringNew As String, ByVal Table As String, ByVal Field As String, ByVal Key1 As String, ByVal Key2 As String, ByVal Key3 As String) As String
        'Hàm mới
        Dim ret As String = ""
        Dim lLastKey As Long = 0
        ret = IGEKeyPrimary_ConnectOther(sConnectionStringNew, Table, Field, Key1, Key2, Key3, lLastKey, 1, OutOrderEnum.lmSSSN, 15, "")
        Return ret

    End Function

    Private Function IGEKeyPrimary_ConnectOther(ByVal sConnectionStringNew As String, _
        ...
        'Update 24/12/2014: Sinh IGE kiểu mới cho dạng Có truyền kết nối khác
        'Chú ý: Chuẩn sinh IGE luôn luôn có dạng SSSN
        Dim ret As String = ""
        ...
        Try
            ...
            Dim dtKey As DataTable
            dtKey = ReturnDataTable(sSQL, sConnectionStringNew)
            If dtKey.Rows.Count > 0 Then
                Dim sKeyString As String = dtKey.Rows(0).Item("KeyString").ToString
                sStringKey3 = sKeyString.Substring(sStringKey1.Length + sStringKey2.Length)
                Dim iKeyFrom As Long = CLng(dtKey.Rows(0).Item("KeyFrom"))
