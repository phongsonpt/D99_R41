...
Public Class D99C1001
    ...
    Public Sub AddFieldName(ByVal sFieldName As String, ByVal sFieldDescription As String, ByVal sDataType As FinderTypeEnum, Optional ByVal nLengh As Integer = 20, Optional ByVal sType As String = "", Optional ByVal NumberFormat As String = "")
        Try

            If FlagAddField OrElse arrAdvanced Is Nothing Then 'Bổ sung vì bị lỗi khi load lần đầu
                ReDim arrAdvanced(0)
                arrAdvanced(UBound(arrAdvanced)) = sFieldDescription & LM_DLM & sFieldName & LM_DLM & sDataType & LM_DLM & CStr(nLengh) & LM_DLM & sType & LM_DLM & NumberFormat
                FlagAddField = False
            Else
    ...
    Public Sub ShowFormFind(ByVal sFormID As String, ByVal sMode As String)
        ...
        Frm.Mode = sMode
        Frm.UseUnicode = _useUnicode
        Frm.FindServer = False ' = True, Sửa lại = False 01/11/2018 Lỗi V41 không còn dùng VNI nữa nhưng qua màn hình F006 Convert Uni -> Uni nên bị lỗi.
        '********************
        Frm.ShowDialog()
        Frm.Dispose()
        FlagAddField = True
    ...
    Private Sub ChangeLanguage()
        m_FormCaption = rL3("Tim_kiemF") & UnicodeCaption(_useUnicode)
        If _Language = EnumLanguage.Vietnamese Then
            ...
            m_DropHeight = 3000
            m_AllowSortOrder = True
        Else 'If _Language = EnumLanguage.English Then'vì chọn tiếng Hoa báo lỗi Object reference khi Tìm kiếm
            FlagAddField = True
    ...
    'Private Function AllOpenForm(ByVal sFormParent As String) As Boolean
    '    Dim frmCollection As FormCollection = Application.OpenForms
    '    For Each frm As Form In frmCollection
    '        If frm.Name = "D99F0006" Then
    '            Dim sFm As Object = GetProperties(frm, "FormID")
    '            If sFm Is Nothing Then Continue For
    '            If sFm.ToString = sFormParent Then CheckExistForm(frm.Name) : Return False
    '        End If
    '    Next
    '    Return True
    'End Function

    Private Sub CallFormD99F00006(ByVal FormParent As Form, ByVal sMode As String, ByVal FindClient As Integer, Optional ByVal bUseV1234 As Boolean = False)
        If Not AllOpenForm(FormParent.Name, "D99F0006", "FormID") Then Exit Sub
        strSQLAdvanced = ""
        strSQLAdvancedForReport = ""
        Dim Frm As New D99F0006
        Frm.LM_FindClient = FindClient
        Frm.dropH = m_DropHeight
        Frm.Text = m_FormCaption
        Frm.ButFind.Text = m_FindCommandCaption
        Frm.GroupBox1.Text = m_FrameCaption
        ...
        'Update 10/03/2009
        Frm.Mode = sMode
        Frm.FormParent = FormParent
        Frm.FormID = FormParent.Name
        'Update 29/07/2010
        Frm.UseUnicode = _useUnicode
        'Update 04/10/2010
        Frm.FindServer = bUseV1234 'Nếu Nhập liệu Unicode thì kiểm tra có tìm kiếm theo view V1234
        Frm.Owner = FormParent
        '********************
        Frm.Show()
        'FlagAddField = True
    ...
    Public Sub ShowFormFindClientServer(ByVal sFormID As Form, ByVal sMode As String, Optional ByVal bUseV1234 As Boolean = False)
        CallFormD99F00006(sFormID, sMode, 2, bUseV1234)
    End Sub

    Public Sub ShowFormFindClient(ByVal sFormID As Form, ByVal sMode As String, Optional ByVal bUseV1234 As Boolean = False)
        CallFormD99F00006(sFormID, sMode, 1, bUseV1234)
    End Sub

    Public Sub ShowFormFind(ByVal sFormID As Form, ByVal sMode As String)
        CallFormD99F00006(sFormID, sMode, 0, False)
    End Sub
End Class
